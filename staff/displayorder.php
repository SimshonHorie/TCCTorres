				...
				if ($currentspan == $rowspan) {
					...
					switch ($orderRow['status']) {
						...
						case 'ready':
							...
							echo "<button onclick='editStatus(this," . $orderRow['orderID'] . ")' class='btn btn-outline-warning' value = 'finish'>Limpar</button>";
...
if ($_GET['cmd'] == 'currentready') {
	...
	if ($result = $sqlconnection->query($latestReadyQuery)) {
		...
		while ($latestOrder = $result->fetch_array(MYSQLI_ASSOC)) {
			echo "<tr><td><i class='fas fa-bullhorn' style='color:green;'></i><b> Ordem #" . $latestOrder['orderID'] . "</b> lista para servir.<a href='editstatus.php?orderID=" . $latestOrder['orderID'] . "'><i class='fas fa-check float-right'></i></a></td></tr>";
		}
	}
}
